...
@property (strong, nonatomic) NSArray<UIView *> *dataViews;
@property (assign, nonatomic) MHSheetStyle sheetStyle;
@end

@implementation MHActionSheet

#pragma mark -- 自定义底部弹出View组 用--
///分享弹框样式
- (instancetype)initSheetWithTitle:(NSString *)title
                             style:(MHSheetStyle)style itemViews:(NSArray<UIView *> *)itemViews{
    if (self = [super initWithFrame:[[UIScreen mainScreen] bounds]]) {
        self.sheetStyle = style;
        self.dataViews = itemViews;
...
//            self = [self upDefaultStyeWithViewGroup:nil title:title selfView:self];//还没有这个方法
            [self pushDefaultStyeSheetView];
        }
        else if (style == MHSheetStyleWeiChat) {
            self = [self upWeiChatStyeWithViewGroup:itemViews title:title selfView:self];
            [self pushWeiChatStyeSheetView];
        }
        else if (style == MHSheetStyleTable) {
//            self = [self upTableStyeWithViewGroup:nil title:title selfView:self];//还没有这个方法
...
#pragma mark -- 自定义底部弹出View 暂--
///分享弹框样式
- (instancetype)initSheetWithTitle:(NSString *)title
                             style:(MHSheetStyle)style customView:(UIView *)customView{
    if (self = [super initWithFrame:[[UIScreen mainScreen] bounds]]) {
        self.sheetStyle = style;
        [[[UIApplication sharedApplication] keyWindow] addSubview:self];
        //半透明背景按钮
        ...
        //选择样式
        if (style == MHSheetStyleDefault) {
//            self = [self upDefaultStyeWithTitle:title selfView:self];//还没有该方法
            [self pushDefaultStyeSheetView];
        }
        else if (style == MHSheetStyleWeiChat) {
//            self = [self upWeiChatStyeWithItemsTitle:title selfView:self];//还没有该方法
            [self pushWeiChatStyeSheetView];
        }
        else if (style == MHSheetStyleTable) {
//            self = [self upTableStyeWithTitle:title selfView:self];//还没有该方法
            [self pushTableStyeSheetView];
        }
    }
    return self;
...
///初始化默认样式
- (instancetype)upDefaultStyeWithItems:(NSArray *)itemTitles title:(NSString *)title selfView:(MHActionSheet *)selfView
{
    //半透明背景按钮
...
///初始化微信样式
- (instancetype)upWeiChatStyeWithItems:(NSArray *)itemTitles title:(NSString *)title selfView:(MHActionSheet *)selfView
{
...
#pragma mark -- 初始化底部弹出View组的样式 --
///初始化底部弹出View组的样式
- (instancetype)upWeiChatStyeWithViewGroup:(NSArray<UIView *> *)viewGroup title:(NSString *)title selfView:(MHActionSheet *)selfView{
    ...
    //布局子控件
    int viewCount = (int)viewGroup.count;
    selfView.contentVH = (kWW/4)*(viewCount/4 + (viewCount%4==0 ? 0 : 1)) + (isTitle ? kCellH : 0) + 2*kMargin;
    ...
    for (int i = 0; i < viewCount; i++) {
        CGRect frame = CGRectMake(i*(kWW/4), (i%4)*(kWW/4), kWW/4, kWW/4);
        UIView *view = [[UIView alloc] initWithFrame:frame];
        UIView *subView = viewGroup[i];
        subView.center = view.center;
        [view addSubview:subView];
        [selfView.contentView addSubview:view];
    }
...
#pragma mark -- 消失分享弹出样式 --
//消失分享弹出样式
- (void)dismissShareStyeSheetView
{
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:kDismissTime animations:^{
        weakSelf.contentView.frame = CGRectMake(0, kWH, kWW, weakSelf.contentVH);
